...
export default function Login() {
  ...
  const handleLogin = async () => {
    if (!user.email || !user.password) return setMessage("Please enter both email and password.");
    setIsLoading(true);
    try {
      ...
      if (!response.ok) return setMessage(data.error || "Login failed. Please try again.");
  ...
  return (
    <div className="flex justify-center md:justify-start items-center h-screen bg-gray-100 dark:bg-gray-300 px-4">
      ...
      >
        ...
        >
          <h1 className="text-5xl md:text-4xl font-bold text-sky-600 dark:text-sky-900 mb-6 text-center">Login</h1>
          <div className="mb-4">
            <label className="block text-gray-700 text-[2em] md:text-[1.5em] font-medium mb-2" htmlFor="email">Email</label>
            <input id="email" name="email" type="email" placeholder="Enter your email" value={user.email} onChange={handleChange} className="w-full p-4 border text-[2em] md:text-[1.5em] border-gray-300 rounded focus:outline-none focus:border-blue-500" />
          </div>
          <div className="mb-6">
            <label className="block text-gray-700 text-[2em] md:text-[1.5em] font-medium mb-2" htmlFor="password">Password</label>
          ...
          <p className="mt-6 text-center text-gray-600 text-[2em] md:text-[1.5em]">Don&apos;t have an account?{" "}
            <Link href="/register" className="text-blue-600 dark:text-sky-900 hover:underline">Register</Link>
          </p>
        </motion.div>
        <motion.div
          ...
          transition={{ duration: 0.5 }}
        >
          <Image src="/login.svg" alt="Login illustration" width={350} height={350} className="lg:w-[450px] lg:h-[450px]" />
        </motion.div>
      </motion.div>
    </div>
  );
